...
char *strToBin(unsigned char *str, size_t sLen)
{
    if (str == NULL || sLen == 0)
        return NULL;
        
    char *binStr = (char*) malloc(sizeof(char) * sLen * 8);
    size_t binPos = sLen * 8 - 1;
    
    for (size_t i = 0; i < sLen; ++i)
    {
        // Convert each char to binary
        int tmp = str[sLen - i - 1];
        for (size_t j = 0; j < 8; ++j)
        {
            binStr[binPos--] = (tmp % 2 == 0) ? '0' : '1';
            tmp >>= 1;
        }
    }
